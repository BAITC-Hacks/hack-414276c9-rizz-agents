...
function render() {
  ...
  $('spent').textContent=cost; $('budgetStatus').textContent=`Осталось ${budget-cost}`; $('budgetBar').style.width=`${Math.min(100,cost/budget*100)}%`; $('budgetBar').style.background=cost>budget?'#d56b57':'';
  ...
  $('calculate').disabled=false; $('calculate').style.opacity='1';
  $('calculate').innerHTML=state.selected.length===5?'Рассчитать сценарий <span>→</span>':`Выбрать ещё ${5-state.selected.length} мер <span>→</span>`;
}
